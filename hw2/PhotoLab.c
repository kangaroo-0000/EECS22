...
void ColorFilter(unsigned char R[WIDTH][HEIGHT],
    ...
    for(int c = 0; c < 512; c++){
        for(int r = 0; r < 288; r++){
	    if((R[c][r] < (target_r + threshold)) && (R[c][r] > (target_r - threshold))&&(G[c][r] < (target_r + threshold)) && (G[c][r] > (target_r - threshold))&&(B[c][r] < (target_r + threshold)) && (B[c][r] > (target_r - threshold))){
...
void Shuffle(unsigned char R[WIDTH][HEIGHT],
	...
	for(int i = 0; i < 128; i++){
		...
		R[(r1%4)*128+i][(r1/4)*72+j] = R[(r2%4)*128+i][(r2/4)*72+j];;
		R[(r2%4)*128+i][(r2/4)*72+j] = temp;
		temp = G[(r1%4)*128+i][(r1/4)*72+j];
		G[(r1%4)*128+i][(r1/4)*72+j] = G[(r2%4)*128+i][(r2/4)*72+j];
		G[(r2%4)*128+i][(r2/4)*72+j] = temp;
		temp = B[(r1%4)*128+i][(r1/4)*72+j];
		B[(r1%4)*128+i][(r1/4)*72+j] = B[(r2%4)*128+i][(r2/4)*72+j];
		B[(r2%4)*128+i][(r2/4)*72+j] = temp;
	    }
	}
    }
...
void AddBorder(unsigned char R[WIDTH][HEIGHT],
    ...
    border_width1 = border_width*16/9;
	...
	printf("you have picked a color that is not on the list. Try again.\n");
	return;
    }	
    ...
    for(int i = 0; i < 512; i++){
	for(int j = 0; j < border_width1; j++){
	    R[i][j] = x;
            G[i][j] = y;
            B[i][j] = z;
	}
    }
    for(int i = 511; i > 512 - border_width; i--){
	for(int j = 0; j < 288; j++){
	    R[i][j] = x;
            G[i][j] = y;
            B[i][j] = z;
    ...
    for(int i = 0; i < 512; i++){
	for(int j = 287; j > 287 - border_width1; j--){
	    R[i][j] = x;
            G[i][j] = y;
            B[i][j] = z;	    
	}
